...
const RecentPRsSection = ({ pullRequests }: RecentPRsSectionProps) => {
  ...
  const triplePRs = [...pullRequests, ...pullRequests, ...pullRequests];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          ...
          <div className="lg:col-span-8 relative h-[600px] overflow-hidden group">
            ...
            <motion.div
              ...
              animate={{
                y: [0, -2000] // Arbitrary large number for height
              }}
              transition={{
                duration: 60,
                repeat: Infinity,
            ...
            >
              {triplePRs.map((pr, index) => (
                <motion.article
                  key={`${pr.number}-${index}`}
                  className="flex flex-col gap-4 rounded-3xl bg-[#F1F3F7]/40 p-5 border border-black/[0.02] hover:bg-white hover:shadow-[0_15px_40px_rgba(0,0,0,0.05)] hover:border-[#149A9B]/20 transition-all duration-300"
                >
