...
export function SignInPage() {
  const { loginWithEmail, isLoading, error, errorDetails, clearError } = useRegister();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: LoginWithEmailPayload = { email, password };
    await loginWithEmail(payload);
  };

  const handleGoogleSignIn = () => initiateOAuth("google");
  const handleGithubSignIn = () => initiateOAuth("github");
      ...
      >
        <Link href="/" className="flex items-center gap-2">
          <Image
            ...
            priority
          />
          <span className="text-lg font-semibold text-gray-900">WARDWORK</span>
        </Link>
      </motion.div>
        ...
        >
          <div className="bg-white rounded-2xl shadow-xl shadow-gray-200/50 p-8 border border-gray-100">
            ...
            <p className="mt-6 text-center text-gray-600">
              ...
              <Link href="/onboarding/sign-up" className="text-[#149A9B] font-semibold hover:text-[#0d7377] transition-colors">
                Sign up
              </Link>
            </p>
          </div>
