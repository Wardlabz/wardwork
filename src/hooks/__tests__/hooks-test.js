/**
 * Test simple para verificar que los hooks están funcionando
  ...
  beforeEach(() => {
    ...
    mockContractCall.mockImplementation((contract, method, args) => {
      ...
      if (contract === 'userRegistry') {
        return Promise.resolve(mockContractResponses.userRegistry[method]);
      } else if (contract === 'escrow') {
        return Promise.resolve(mockContractResponses.escrow[method]);
      } else if (contract === 'rating') {
        return Promise.resolve(mockContractResponses.rating[method]);
      } else if (contract === 'publication') {
        return Promise.resolve(mockContractResponses.publication[method]);
      } else if (contract === 'dispute') {
        return Promise.resolve(mockContractResponses.dispute[method]);
      }
