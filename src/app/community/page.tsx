...
async function fetchGitHubData() {
  try {
    const [repoResponse, contributorsResponse, pullRequestsResponse, issuesResponse] = await Promise.all([
      fetch('https://api.github.com/repos/WARDWORK/wardwork-monorepo', {
        next: { revalidate: 3600 },
      }),
      fetch('https://api.github.com/repos/WARDWORK/wardwork-monorepo/contributors', {
        next: { revalidate: 3600 },
      }),
      fetch('https://api.github.com/repos/WARDWORK/wardwork-monorepo/pulls?state=closed&sort=updated&per_page=20', {
        next: { revalidate: 3600 },
      }),
      fetch('https://api.github.com/repos/WARDWORK/wardwork-monorepo/issues?state=open&per_page=20', {
        next: { revalidate: 3600 },
      }),
    ]);
