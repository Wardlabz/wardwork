...
export async function sendEmail(message: EmailMessage): Promise<void> {
  try {
    ...
    const from = process.env.EMAIL_FROM || 'noreply@wardwork.org';
...
export async function sendPasswordResetEmail(
  ...
  const html = `
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
      <div style="background-color: #f8f9fa; padding: 30px; border-radius: 8px; margin-bottom: 20px;">
        ...
        <p>Hello,</p>
        <p>We received a request to reset your password for your WardWork account. Click the button below to reset your password:</p>
        ...
        <p style="color: #999; font-size: 12px; margin-bottom: 0;">
          This is an automated message from WardWork. Please do not reply to this email.
        </p>
      </div>
    </body>
    </html>
  ...
  await sendEmail({
    to: email,
    subject: 'Reset Your WardWork Password',
    html,
  });
}
...
export async function sendAccountDeletionEmail(email: string): Promise<void> {
  const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
  const supportEmail = process.env.SUPPORT_EMAIL || 'support@wardwork.org';
  ...
  const html = `
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
      <div style="background-color: #f8f9fa; padding: 30px; border-radius: 8px; margin-bottom: 20px;">
        ...
        <p>Hello,</p>
        <p>We're writing to confirm that your WardWork account has been scheduled for deletion.</p>
        ...
        <hr style="border: none; border-top: 1px solid #eee; margin: 30px 0;">
        <p style="color: #999; font-size: 12px; margin-bottom: 0;">
          This is an automated message from WardWork. This email was sent to confirm account deletion.
        </p>
      </div>
    </body>
    </html>
...
export const emailService = {
  ...
  sendWithdrawalRefundEmail,
    ...
    subject: 'WardWork Account Deletion Confirmation',
    html,
  });
}
