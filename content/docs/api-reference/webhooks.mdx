...
Webhooks let your application receive real-time notifications when events occur in WARDWORK — order updates, balance credits, escrow state changes, and more.
...
Every webhook request includes an `X-WardWork-Signature` header. Verify it to confirm the payload came from WARDWORK:
...
WARDWORK retries failed webhook deliveries with exponential back-off:
