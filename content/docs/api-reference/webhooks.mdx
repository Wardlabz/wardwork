---
title: Webhooks
description: Receive real-time event notifications from WardWork via HTTP webhooks.
order: 2
...
Webhooks let your application receive real-time notifications when events occur in WardWork — escrow lifecycle changes, balance updates, order state changes, and more. You register an HTTP endpoint; WardWork sends a `POST` request with a JSON payload and an `X-WardWork-Signature` header so you can verify authenticity.
...
```bash
curl -X POST http://localhost:4000/api/v1/webhooks -H "Content-Type: application/json" -d '{"url": "https://your-app.com/webhooks/wardwork", "events": ["escrow.created", "escrow.released"], "secret": "your-signing-secret"}'
```
...
```json
{
  "url": "https://your-app.com/webhooks/wardwork",
...
WardWork emits the following webhook events. Subscribe only to the ones your integration needs.
...
Every webhook request includes an `X-WardWork-Signature` header. Verify it using your webhook secret and the raw request body to ensure the payload came from WardWork and was not modified.
...
// const isValid = verifyWebhookSignature(req.rawBody, req.headers["x-wardwork-signature"], secret);
```
...
WardWork retries failed webhook deliveries with exponential back-off. A delivery is considered failed if your endpoint returns a non-2xx status or does not respond within 10 seconds.
...
Your development server is not reachable from the internet. Use a tunnel to expose it so WardWork can deliver webhooks.
