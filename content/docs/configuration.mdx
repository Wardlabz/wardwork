...
WardWork Orchestrator is configured through environment variables in `.env`. This page documents every available option.
...
| Variable | Required | Example | Description |
|----------|----------|---------|-------------|
| `WARDWORK_MASTER_KEY` | Yes | `wwk_master_xxx` | Master key for creating API keys |
...
NEXT_PUBLIC_SITE_URL=https://wardwork.tech
```
...
# Authentication
WARDWORK_MASTER_KEY=change-me-in-production
...
3. **Rotate keys periodically** - Especially `WARDWORK_MASTER_KEY`
