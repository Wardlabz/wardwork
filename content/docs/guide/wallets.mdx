---
title: Wallets Guide
description: Understanding invisible wallets in WardWork — automatic Stellar wallet creation and management.
order: 5
section: Guides
---

<Callout type="note">
  This is the public API/SDK guide for integrators. For internal implementation detail (Orchestrator state machine, signer roles), see [`docs/guides/wallets.md`](https://github.com/Wardlabz/wardwork-monorepo/blob/main/docs/guides/wallets.md) in the repo.
</Callout>

WardWork uses "invisible wallets" to give users blockchain capabilities without the complexity of managing private keys. Every user gets a Stellar wallet automatically, but they interact with it like a simple account balance.
...
WardWork eliminates all of this. Users see a simple USD balance while the system manages Stellar wallets behind the scenes.
...
WardWork maintains both on-chain (Stellar) and off-chain (database) balances:
