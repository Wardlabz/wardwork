---
title: Orchestrator
description: Understand WARDWORK's core payment orchestration architecture and how it processes transactions.
order: 1
section: Guides
---

The Orchestrator is the central engine of WARDWORK. It coordinates every step of a transaction — from the moment a buyer deposits USDC to the moment a seller withdraws funds — enforcing rules, managing state, and communicating with the Stellar blockchain so your marketplace never has to.
...
flowchart TB
    ...
    subgraph Orchestrator["WARDWORK Orchestrator"]
...
<Callout variant="note">
  At this point funds are held by the smart contract — not by WARDWORK. The Orchestrator cannot unilaterally move them. Only valid contract calls can release, dispute, or refund.
</Callout>
