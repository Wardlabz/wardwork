---
title: Escrow Guide
description: How blockchain-based escrow works in WardWork — fund locking, release flows, and dispute handling.
order: 3
section: Guides
---

<Callout type="note">
  This is the public API/SDK guide for integrators. For internal implementation detail (Orchestrator state machine, signer roles), see [`docs/guides/escrow.md`](https://github.com/Wardlabz/wardwork-monorepo/blob/main/docs/guides/escrow.md) in the repo.
</Callout>

Escrow is the core security mechanism in WardWork. When a buyer pays for a service, funds are locked in a Stellar smart contract until the work is delivered and approved. Neither party can unilaterally access the funds — the contract enforces fair resolution.
...
WardWork uses [Trustless Work](https://trustlesswork.com) smart contracts on the Stellar blockchain for non-custodial escrow. This means:

1. **Funds are on-chain** — Not in WardWork's database
2. **Contract enforces rules** — Release requires proper signatures
3. **Transparent** — Anyone can verify escrow state on Stellar
4. **Non-custodial** — WardWork cannot access funds without proper authorization
...
| **Platform** | WardWork's signer for dispute resolution |
...
WardWork tracks two types of balances:
