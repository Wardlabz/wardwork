---
title: Self-Hosting
description: Complete instructions for deploying WardWork Orchestrator on your own infrastructure using Docker.
order: 8
section: Guides
---

Run WardWork Orchestrator on your own server with full control over data, uptime, and configuration. This guide covers everything from initial setup to ongoing maintenance using Docker.
...
The WardWork deployment consists of:
...
<CommandLine>mkdir wardwork-deploy && cd wardwork-deploy</CommandLine>
...
services:
  postgres:
    ...
    ports:
      - "5432:5432"
    environment:
      POSTGRES_USER: wardwork
      POSTGRES_PASSWORD: ${POSTGRES_PASSWORD:-wardwork_secret}
      POSTGRES_DB: wardwork_db
    volumes:
      - postgres_data:/var/lib/postgresql/data
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U wardwork"]
      interval: 10s
      timeout: 5s
      retries: 5
  ...
  orchestrator:
    image: wardwork/orchestrator:latest
    ports:
      - "4000:4000"
    env_file:
      - .env
    environment:
      - DATABASE_URL=postgresql://wardwork:${POSTGRES_PASSWORD:-wardwork_secret}@postgres:5432/wardwork_db
  ...
  frontend:
    image: wardwork/frontend:latest
    ports:
      - "3000:3000"
    environment:
...
<CommandLine>git clone https://github.com/Wardlabz/wardwork-orchestrator.git && cd wardwork-orchestrator</CommandLine>

<CommandLine>docker build -t wardwork/orchestrator:latest .</CommandLine>
...
| `WARDWORK_MASTER_KEY` | Master key for creating API keys |
...
# Database (PostgreSQL)
DATABASE_URL=postgresql://wardwork:your_password@postgres:5432/wardwork_db
...
# Authentication
WARDWORK_MASTER_KEY=your-secure-master-key-at-least-32-chars
...
```bash
# 1. Backup database
docker compose exec postgres pg_dump -U wardwork wardwork_db > backup-$(date +%Y%m%d).sql
...
services:
  orchestrator:
    image: wardwork/orchestrator:1.2.0   # pin to a known-good version
```
...
<Callout type="note">
  Check the [GitHub Releases](https://github.com/Wardlabz/wardwork-orchestrator/releases) page for available tags and changelogs.
</Callout>
...
- [ ] `WARDWORK_MASTER_KEY` is a strong, unique value
