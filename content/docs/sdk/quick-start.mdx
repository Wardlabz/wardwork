...
description: Get started with the WardWork TypeScript SDK in minutes.
order: 1
section: SDK
---

The WardWork SDK provides a type-safe TypeScript client for integrating with the Orchestrator API. It handles authentication, retries, and provides typed errors for better error handling.
...
<CommandLine>yarn add @wardwork/sdk</CommandLine>
...
const sdk = new WardWorkSDK({
  apiUrl: process.env.WARDWORK_API_URL,    // e.g., http://localhost:4000
  apiKey: process.env.WARDWORK_API_KEY,    // e.g., wwk_live_xxx
...
```env
WARDWORK_API_URL=http://localhost:4000
WARDWORK_API_KEY=wwk_live_your_api_key_here
```
...
```typescript
import type { User, Order, Balance } from '@wardwork/sdk';
